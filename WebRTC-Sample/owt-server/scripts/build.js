...
const owtRootDir = path.join(rootDir, 'third_party/owt-server/');
const depsDir = path.join(owtRootDir, 'build/libdeps/build/');

const { chdir, cwd } = process;
const { execSync } = require('child_process');
const { OptParser, exec } = require(owtRootDir + 'scripts/util');
...
Promise.all(works)
  .then(() => {
    if (options.check) {
      moduleTestScript = path.join(owtRootDir, 'scripts/module_test.js');
      runtimeAddonDir = path.join(rootDir, 'source');
