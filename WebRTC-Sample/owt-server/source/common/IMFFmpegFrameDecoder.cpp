...
#include "IMFFmpegFrameDecoder.h"
...
void FFmpegFrameDecoder::onFrame(const Frame& frame)
{
    ...
    if (isVideoFrame(frame)) {
        LTTNG_TRACE_INFO("FFmpegFrameDecoder: Receive frame, stream(%ld), pts(%d), size(%d), %s"
                , (long int)this
                , frame.timeStamp /90
                , frame.length
                , frame.additionalInfo.video.isKeyFrame ? "key" : "non-key");
    }
    ...
    while(true) {
...

        {
            ...
            LTTNG_TRACE_INFO("FFmpegFrameDecoder: Deliver frame, stream(%ld), pts(%ld), width(%d), height(%d)"
                    , (long int)this
                    , m_decFrame->pts / 90
                    , video_frame->width()
                    , video_frame->height());
