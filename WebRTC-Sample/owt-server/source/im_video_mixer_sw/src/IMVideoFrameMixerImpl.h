                                           ...
                                           owt_base::FrameDestination* dest)
{
    ...
    if (it != m_outputs.end()) { // Found a reusable encoder
        ...
        if (!encoder && format == owt_base::FRAME_FORMAT_H265) {
            if (isHEVCMCTSVideoResolution(m_rootSize.width, m_rootSize.height) &&
                isHEVCMCTSVideoResolution(outputSize.width, outputSize.height))
