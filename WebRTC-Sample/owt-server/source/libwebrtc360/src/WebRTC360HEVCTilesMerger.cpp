...
#define LTTNG_TRACE_INFO    ELOG_DEBUG

#ifdef ENABLE_WEBRTC360
#define ELOG_DEBUG printf
#define ELOG_ERROR printf
#define ELOG_INFO printf
#define ELOG_TRACE printf
#endif

#ifndef ENABLE_WEBRTC360
namespace owt_base {
...
void HEVCTilesMerger::onFrame(const Frame& frame, Frame *pOutFrame)
{
    ...
    if (m_handle) {
        ...
        m_360scvp_param.timeStamp = tilesFrame.timeStamp;
        ...
        if (tilesFrame.isKey) {
...

            {
                ...
                if (m_fovUpdated) {
                    ...
                    LTTNG_TRACE_INFO("HEVCTilesMerger: Apply new FoV, stream(%ld), yaw(%d), pitch(%d)", (long int)this, m_yaw+180, m_pitch);
                    m_360scvp_param.paramViewPort.viewPortYaw = m_yaw;
                    m_360scvp_param.paramViewPort.viewPortPitch = m_pitch;
