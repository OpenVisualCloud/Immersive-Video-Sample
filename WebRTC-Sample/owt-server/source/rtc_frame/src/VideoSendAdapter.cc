...
void VideoSendAdapterImpl::onFrame(const Frame& inFrame)
{
    Frame frame = inFrame;
...
//for rtcp fov
void VideoSendAdapterImpl::OnReceivedFOVFeedback(webrtc::RtcpFOVInfo &rtcp_fov_info) {
    RTC_DLOG(LS_INFO) << "OnReceivedFOVFeedback, seq_nr " << rtcp_fov_info.seqnr
      << " yaw: " << rtcp_fov_info.yaw << " pitch: " << rtcp_fov_info.pitch;
...
//for http fov
void VideoSendAdapterImpl::setFoV(int32_t yaw, int32_t pitch) {
#ifdef _ENABLE_HEVC_TILES_MERGER_
    if (m_tilesMerger)
    {
